...
public class PlayerStats : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < characterData.AllWeapons.Count; i++)
        {
            GetComponent<PlayerInventory>().availableWeapons.Add(characterData.AllWeapons[i]);
        }
    ...
    void UpdateExpBar()
    {
        ...
        expBar.value = (float)experience / experienceCap;
    ...
    public void TakeDamage(float dmg)
    {
        ...
        if (!isInvincible)
        {
            ...
            if (dmg > 0)
            {
                ...
                if (damageEffect) 
                {
                   ParticleSystem dmgPref = Instantiate(damageEffect, transform.position, Quaternion.identity);
                    dmgSfx.Play();
                    Destroy(dmgPref, 2.5f);
                } 
            ...
            else
            {
                ...
                if (blockedEffect) Destroy(Instantiate(blockedEffect, transform.position, Quaternion.identity), 2.5f);
            }
    ...
    void UpdateHealthBar()
    {
        ...
        healthBar.value = CurrentHealth / actualStats.maxHealth;
        lifeText.text = "" + Mathf.Round(CurrentHealth).ToString() + "/" + Mathf.Round(actualStats.maxHealth).ToString();
    }
